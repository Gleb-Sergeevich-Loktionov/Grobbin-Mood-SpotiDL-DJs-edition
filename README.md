...
CLI-приложение для скачивания Spotify-плейлистов: берёт треки из Spotify через Spotify Web API, ищет подходящие версии на YouTube, скачивает аудио через `yt-dlp`, конвертирует через FFmpeg и раскладывает файлы по папкам плейлистов.
